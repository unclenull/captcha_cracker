...
from generator import MixGenerator
...
class CycleGAN():
    def __init__(self, lr_D=2e-4, lr_G=2e-4, image_shape=(256*1, 256*1, 1),
        ...
        if self.use_data_generator:            
            # self.data_generator = load_data.load_data(nr_of_channels=self.channels, batch_size=self.batch_size, generator=True, subfolder=image_folder)
            self.data_generator = MixGenerator(self.batch_size)
        ...
        for epoch in range(1, epochs + 1):
            if self.use_data_generator:
                ...
                for images in self.data_generator:
                    ...
                    if loop_index >= self.data_generator.count:
                        break
...
# Save and load
    ...
    def writeMetaDataToJSON(self):
        ...
        data['meta_data'].append({
            ...
            # 'number of A train examples': len(self.data_generator.train_A) if self.use_data_generator else len(self.A_train),
            # 'number of B train examples': len(self.data_generator.train_B) if self.use_data_generator else len(self.B_train),
...
if __name__ == '__main__':
    GAN = CycleGAN(image_shape=(52, 100, 1))
