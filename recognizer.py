...
from utils import parse_args as base_parse_args, get_recognizer_generator, show_metrics, show_test
...
def train(flags=None):
    ...
    gens, img_shape, count = _gen_data(True)
    if gens is None:
        print('no dataset')
        exit()
    else:
        (train_gen, test_gen) = gens
        steps_train = ceil(count / FLAGS.batch_size)
        steps_test = ceil(count / FLAGS.batch_size)
